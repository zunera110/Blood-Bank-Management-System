...
public class DeleteQueryServlet extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if (rowsAffected > 0) {
                response.sendRedirect("ManageQueryServlet?message=Query Deleted Successfully");
            } else {
                response.sendRedirect("ManageQueryServlet?error=Failed to Delete Query");
            }
            con.close();
        } catch (Exception e) {
            e.printStackTrace();
